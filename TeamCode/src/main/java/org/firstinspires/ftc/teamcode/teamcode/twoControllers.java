...
public class twoControllers extends LinearOpMode {
    ...
    public Servo claw, wrist, clawMovement; // non continuous servo initialization
    ...
    public void runOpMode() {
        ...
        wrist= hardwareMap.get(Servo.class, "wrist");              //port 2 on control
        ...
        while (opModeIsActive()) {
                ...
                if (gamepad2.right_trigger > 0) { //extends slides
                    if (leftSlidePos < 1200) { //if within base and 2500
                        rightExtend.setPower(1);
                        leftExtend.setPower(-1);
                    }
                } else if (gamepad2.left_trigger > 0) { //retracts slides
                    rightExtend.setPower(-1);
                    leftExtend.setPower(1);
                } else {
    ...
    */
                ...
                if (gamepad2.y) { //move claw to score
                    clawMovement.setPosition(1);
                }

                if (gamepad2.x){
                    clawMovement.setPosition(0.75);
                }

                if (gamepad2.b) { //move claw down
                    clawMovement.setPosition(0.25);
                }
                if (gamepad2.left_stick_y > 0) { //0 degreed
                    wrist.setPosition(0);
                }
                if (gamepad2.left_stick_y < 0 ){ //180 degrees
                    wrist.setPosition(0.6);
                }
                if (gamepad2.left_stick_x < 0) { //90 degrees
                    wrist.setPosition(0.3);
                }
                if (gamepad2.dpad_right) { // 45 degrees
                    wrist.setPosition(0.15);
                }
                if (gamepad2.dpad_left) { //135 degrees
                    wrist.setPosition(0.45);
                }

                if (gamepad1.y) { //forwards slowly
                    rightFront.setPower(0.4);
                    leftFront.setPower(0.4);
                    rightBack.setPower(0.4);
                    leftBack.setPower(0.4);

                } else if (gamepad1.a) { //backwards slowly
                    rightFront.setPower(-0.4);
                    leftFront.setPower(-0.4);
                    rightBack.setPower(-0.4);
                    leftBack.setPower(-0.4);
